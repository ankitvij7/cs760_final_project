...
mood_values_to_ordinals = dict()
n = 0
for k in mood_labels.keys():
    mood_values_to_ordinals[mood_labels[k]] = n
...
def extract_record_to_xy(record, mood_value):
    ...
    audio_frame += pad
    return audio_frame, mood_values_to_ordinals[mood_value]
...
def load_records(csv_infile, input_tfr_prefix):
    input_tfr_suffix = '.tfrecord'
    records = []
    print("Loading records from '" + csv_infile + "'...")
    with open(csv_infile, 'r') as csvfile:
        reader = csv.reader(csvfile, quotechar='"', delimiter=' ', quoting=csv.QUOTE_ALL, skipinitialspace=True)
        for row in reader:
            # load the records
            tf_infile = input_tfr_prefix + row[3] + input_tfr_suffix
            for t in tf.python_io.tf_record_iterator(tf_infile):
                r = extract_record_to_xy(t, row[4])
                if r:
                    records.append(r)
...
def main():
    ...
    input_tfr_prefix = 'audioset_v1_embeddings/unbal_train_we_gens1/'
    test_input_file = 'moods_eval.csv'
    test_input_tfr_prefix = 'audioset_v1_embeddings/eval_we_gens1/'
